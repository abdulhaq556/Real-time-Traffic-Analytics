    ...
    # ══════════════════════════════════════════════════════════════════════
        ...
        with c2:
            if pred_prob is not None:
                ...
                st.plotly_chart(fig, use_container_width=True)
        with c3:
            # Idea #4 — Mini map for the selected location, colored by prediction
            st.markdown("<div style='font-size:0.78rem;color:#7c8da0;margin-bottom:6px;font-family:\"IBM Plex Mono\",monospace;'>📍 LOCATION</div>", unsafe_allow_html=True)
            rgb = {"Free_Flow": [74, 222, 128], "Moderate": [250, 204, 21], "Congested": [248, 113, 113]}.get(pred_lbl, [0, 255, 231])
            mini_df = pd.DataFrame([{"lat": lat, "lon": lon}])
            mini_layer_glow = pdk.Layer(
                "ScatterplotLayer", data=mini_df,
                get_position="[lon, lat]", get_fill_color=rgb + [70],
                get_radius=350, pickable=False,
            )
            mini_layer_core = pdk.Layer(
                "ScatterplotLayer", data=mini_df,
                get_position="[lon, lat]", get_fill_color=rgb + [235],
                get_radius=120, pickable=False,
            )
            mini_view = pdk.ViewState(latitude=lat, longitude=lon, zoom=12, pitch=0)
            st.pydeck_chart(pdk.Deck(
                layers=[mini_layer_glow, mini_layer_core],
                initial_view_state=mini_view,
